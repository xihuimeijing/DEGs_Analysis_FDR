...
ls_gtex <- lapply(1:6, function(j){
  ...
  # trueDE = intersect(intersect(intersect(dis_edger, dis_limma), dis_deseq2), re_wilxocon)
  # re_noiseq <- my.NOISeq(dataset, conditions)
  # dis_noiseq <- which(re_noiseq <= 0.000001)
  # dis0 <- readRDS(paste0(celltype.pairs[j], '_dis.rds'))
  # trueDE = intersect(intersect(intersect(dis0[[1]], dis0[[2]]), dis0[[3]]), dis0[[4]])
  # trueDE = intersect(trueDE, dis_noiseq)
  dis0 <- readRDS(paste0(celltype.pairs[j], '_dis.rds'))
  trueDE = intersect(intersect(intersect(intersect(dis0[[1]], dis0[[2]]), dis0[[3]]), dis0[[4]]), dis0[[5]])
  
  ls_onepair <- mclapply(1:50, function(it){
    set.seed(it)
    dataset0 <- dataset
    ...
    count.perm1 <- dataset0[,1:(ncol(count1))]
    count.perm2 <- dataset0[,-(1:(ncol(count1)))]
    thres <- c(0.05, 0.01, 0.001, 0.0001, 0.00001)

    re_edger <- myedger(count.perm1, count.perm2)
    dis_edger <- re_edger[[1]]
    p_edger <- re_edger[[2]]
    fdppow[[1]] <- sapply(1:5, function(j){
      dis0 <- which(p.adjust(p_edger, method = "BH")<=thres[j])
      compute_fdppow(dis0, trueDE0)
    })
    fdppow.i[[1]] <- sapply(1:5, function(j){
    ...
    dis_limma <- re_limma[[1]]
    p_limma <- re_limma[[2]]
    fdppow[[2]] <- sapply(1:5, function(j){
      dis0 <- which(p.adjust(p_limma, method = "BH")<=thres[j])
      compute_fdppow(dis0, trueDE0)
    })
    fdppow.i[[2]] <- sapply(1:5, function(j){
...
saveRDS(ls_gtex, "ls_gtex_noiseq.rds")





ls_tcga <- lapply(1:6, function(j){
  ...
  # re_deseq2 <- mydeseq2(count1, count2, q= 0.000001)
  # dis_deseq2 <- re_deseq2[[1]]
  # #re_wilxocon <- my.wilcoxon(count1, count2)
  # re_wilxocon <- my.wilcoxon(score_exp, score_back)
  # re_wilxocon <- find_discovery_wpval(re_wilxocon, q = 0.000001)[[1]]
  # trueDE = intersect(intersect(intersect(dis_edger, dis_limma), dis_deseq2), re_wilxocon)
  ...
  ls_onepair <- mclapply(1:50, function(it){
    ...
    re_edger <- myedger(count.perm1, count.perm2, q = 0.05)
    dis_edger <- re_edger[[1]]
    p_edger <- re_edger[[2]]
    fdppow[[1]] <- sapply(1:5, function(j){
      dis0 <- which(p.adjust(p_edger, method = "BH")<=thres[j])
    ...
    re_limma <- mylimma(count.perm1, count.perm2, q = 0.05)
    dis_limma <- re_limma[[1]]
    p_limma <- re_limma[[2]]
    fdppow[[2]] <- sapply(1:5, function(j){
      dis0 <- which(p.adjust(p_limma, method = "BH")<=thres[j])
    ...
    re_deseq2 <- try(mydeseq2(count.perm1, count.perm2, q = 0.05))
    if (class(re_deseq2) == "try-error"){
      fdppow[[2]] <- NA
      fdppow.i[[2]] <- NA
    }else{
    ...
    re_wilxocon <- my.wilcoxon(count.perm1, count.perm2)
    #re_wilxocon <- my.wilcoxon(score_exp.perm, score_back.perm)
    fdppow[[4]] <- sapply(1:5, function(j){
      dis0 <- which(p.adjust(re_wilxocon, method = "BH")<=thres[j])
      compute_fdppow(dis0, trueDE0)
    })
...
saveRDS(ls_tcga, "ls_tcga_noiseq.rds")



######################  one_condition   ########################

ls_gtex_one_condition <- lapply(1:6, function(j){
  if (j <=5){
    count.mat <- read.table(paste0(celltype.pairs[j], '.gene_readCount.tsv'), sep = '\t', header = TRUE)
  ...
  ls_onepair <- mclapply(1:50, function(it){
    set.seed(it)
    dataset0 <- count1
    index0 <- sample(ncol(dataset0), ncol(dataset0)/2)
    count.perm1 <- dataset0[,index0]
    count.perm2 <- dataset0[,-index0]
    thres <- c(0.05, 0.01, 0.001, 0.0001, 0.00001)
    
    re_edger <- myedger(count.perm1, count.perm2)
    p_edger <- re_edger[[2]]
    dis_edger <- find_discovery_wpval(pval = p_edger, q = thres)
    l_edger <- sapply(1:5, function(j){length(dis_edger[[j]])})
    re_limma <- mylimma(count.perm1, count.perm2)
    p_limma <- re_limma[[2]]
    dis_limma <- find_discovery_wpval(pval = p_limma, q = thres)
    l_limma <- sapply(1:5, function(j){length(dis_limma[[j]])})
    
    re_deseq2 <- try(mydeseq2(count.perm1, count.perm2))
    if (class(re_deseq2) == "try-error"){
      l_limma <- rep(NA, 5)
    }else{
      p_deseq2 <- re_deseq2[[2]]
      l_deseq2 <- sapply(1:5, function(j){length(which(re_deseq2[[2]] <= thres[j]))})
    }
   
    re_wilcoxon <- my.wilcoxon(count.perm1, count.perm2)
    dis_wilcoxon <- find_discovery_wpval(pval = re_wilcoxon, q = thres)
    l_wilcoxon <- sapply(1:5, function(j){length(dis_wilcoxon[[j]])})
    dat1 = data.frame(l_edger, l_limma, l_deseq2, l_wilcoxon)
    ...
    dis_wilcoxon <- find_discovery_wpval(pval = re_wilcoxon, q = thres)
    l_wilcoxon <- sapply(1:5, function(j){length(dis_wilcoxon[[j]])})
    dat2 = data.frame(l_edger, l_limma, l_deseq2, l_wilcoxon)
    return(list(dat1, dat2))
  }, mc.cores = 25)
  return(ls_onepair)
})
saveRDS(ls_gtex_one_condition, "ls_gtex_one_condition.rds")
...
ls_tcga <- lapply(1:6, function(j){
  ...
  ls_onepair <- mclapply(1:50, function(it){
    ...
    #dataset0 <- dataset0[, sample(1:ncol(dataset), ncol(dataset))]
    count.perm1 <- dataset0[,1:(ncol(score_exp))]
    count.perm2 <- dataset0[,-(1:(ncol(score_exp)))]
    thres <- c(0.05, 0.01, 0.001, 0.0001, 0.00001)
...
saveRDS(ls_tcga, "ls_tcga_0516.rds")